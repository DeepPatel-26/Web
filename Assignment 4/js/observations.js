...
function titleCase(s) {
  ...
  for (i = 0; i < Fname.length; i++) {
    Fname[i] = Fname[i][0].toUpperCase() + Fname[i].substring(1);
  }
  return Fname.join(" ");
}
...
function transformObservations(observations) {
  return observations.map(function (observation) {
    ...
    (Obj.photoUrl = observation.taxon.default_photo.square_url),
      ...
      (Obj.isNative = observation.taxon.native == true);
    Obj.isIntroduced = observation.taxon.introduced == true;
    Obj.isEndangered = observation.taxon.endangered == true;
    Obj.isThreatened = observation.taxon.threatened == true;
    return Obj;
  });
}
